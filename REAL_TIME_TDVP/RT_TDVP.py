...
def imag_time_evol (H0, psi, g, dt, steps, maxdim, maxdim_psi2, cutoff_mps, cutoff_mps2, krylovDim):
    psi = copy.copy(psi)
    psi2 = psi_sqr (psi)
    enss = []
    ts = []
    ...
    for n in range(steps):
        ...
        psi_dim.append(np.max(npmps.MPS_dims(psi)))
        print(f'npmps.MPS_dims(psi2):',np.max(npmps.MPS_dims(psi2)))    
        enss.append(np.real(ens[-1]))
        r_pos = np.sqrt(npmps.inner_MPO (psi, psi, rsqr_mpo))
        pos_val.append(r_pos)
        time_val.append(np.abs(dt)*n)
    np.savetxt('psi2_dim.txt', psi2_dim, fmt='%d')
    np.savetxt('psi_dim.txt', psi_dim, fmt='%d')
    return psi, enss, ts, pos_val, time_val
...
if __name__ == '__main__':    
    ...
    #Lz[0] *= -1*omega
    H0 = H_SHO
    # initial mesure x**2 + y**2 mpo
    rsqr_mpo = sho.xsq_measure(N,x1,x2)
    rsqr_mpo = npmps.get_H_2D (rsqr_mpo)
    ...
    with open('tci_initial.pkl', 'wb') as f:
        pickle.dump(psi, f)
    psi_TDVP, ens_TDVP, ts0, pos_tdvp, time_tdvp  = imag_time_evol (H0, psi, g, dt, steps=steps, maxdim=maxdim, maxdim_psi2=maxdim_psi2, cutoff_mps=cutoff_mps, cutoff_mps2=cutoff_mps2, krylovDim=krylovDim)
    TDVP_CPUTIME = np.column_stack((ts0, ens_TDVP))
    TIME_POSI = np.column_stack((time_tdvp, pos_tdvp))
    np.savetxt('TDVP_CPUTIME.txt', TDVP_CPUTIME, fmt='%.12f')
    np.savetxt('position_val.txt', TIME_POSI, fmt='%.12f')
    with open('quench_tdvp.pkl', 'wb') as f:
