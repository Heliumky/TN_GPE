...
plt.rcParams.update({'font.size': 12})          # base font size for all text
...
# ============================================
mat_data = sio.loadmat('GPELab_BESP_results.mat',
...
# ============================================
mat_FD_data = sio.loadmat('GPELab_BEFD_results.mat',
                          struct_as_record=False, squeeze_me=True)
...
for step in range(interval, steps + 1, interval):
    ...
    try:
        ...
        print(f" Cannot read {
              filename} or its companion files. Skipping step {step}.")
        continue
...
avgT_gd = avg_cpu_time(sweep_cput)
avgT_besp = avg_cpu_time(CPU_time_mat)
avgT_befd = avg_cpu_time(CPU_time_mat_FD)
avgT_ite = avg_cpu_time(cput_log)
...
def prepare_density(psi, name="", default_L=42.0):
    ...
    if ndim == 2:
        ...
        if side * side == N:
            ...
            X, Y = np.meshgrid(x, y)
            print(f"{name}: reshaped 1D array of length {
                  N} to {side}x{side} grid.")
            return X, Y, density_2d
        else:
            print(f"{name}: 1D array of length {
                  N} cannot be reshaped to square. Using 1D plot fallback.")
...
density_bench = None
try:
    bench_data = sio.loadmat(
        benchmark_file, struct_as_record=False, squeeze_me=True)
    ...
    X_bench, Y_bench, Z_bench = prepare_density(
        phi_bench, name="Benchmark", default_L=Lx_bench)
...
# ============================================
# Reduced figure size for better scaling
fig, axs = plt.subplots(2, 4, figsize=(13, 6.5))
#plt.subplots_adjust(wspace=0.2, hspace=0.3)

axs[0, 1].sharey(axs[0, 0])   # (b) share (a)
axs[0, 3].sharey(axs[0, 2])   # (d) share (c)
...
axs[0, 0].scatter(sweep_cput[::ite], np.abs(sweep_mu)[::ite], c='r', s=4,
                  label=f'QTT GD')
axs[0, 0].scatter(sweep_cput[799], np.abs(sweep_mu)[799], c='r', s=96)

axs[0, 0].scatter(CPU_time_mat[::ite], np.abs(mu_mat)[::ite], c='b', s=4,
                  label=f'BESP')
axs[0, 0].scatter(CPU_time_mat[799], np.abs(mu_mat)[799], c='b', s=96)

axs[0, 0].scatter(CPU_time_mat_FD[::ite], np.abs(mu_mat_FD)[::ite], c='g', s=4)
axs[0, 0].scatter(CPU_time_mat_FD[799], np.abs(mu_mat_FD)[799], c='g', s=96)

axs[0, 0].scatter(cput_log[::ite], np.abs(mu_log)[::ite], c='k', s=4)
axs[0, 0].scatter(cput_log[799], np.abs(mu_log)[799], c='k', s=96)

axs[0, 0].set_xlabel(r'CPU time (s)', fontsize=20, ha='left', x=-0.05)
axs[0, 0].set_xlim(0, 1e4)
axs[0, 0].set_ylim(2.87, 3.80)
axs[0, 0].set_ylabel(r'$\mathrm{\mu}$', fontsize=20)
...
axs[0, 0].text(0.05, 0.85, "(a)", fontsize=28, transform=axs[0, 0].transAxes)
axs[0, 0].legend(loc='upper right', fontsize=14,
                 bbox_to_anchor=(1.03, 1.03), markerscale=2)

# (b) μ error vs STEP
axs[0, 1].scatter(range(len(sweep_cput))[::ite], np.abs(sweep_mu)[::ite], c='r', s=4,
                  label=f'QTT GD (avgT={avgT_gd:.2f} s)')
axs[0, 1].scatter(range(len(CPU_time_mat))[::ite], np.abs(mu_mat)[::ite], c='b', s=4,
                  label=f'BESP (avgT={avgT_besp:.2f} s)')
axs[0, 1].scatter(range(len(CPU_time_mat_FD))[::ite], np.abs(mu_mat_FD)[::ite], c='g', s=4,
                  label=f'BEFD (avgT={avgT_befd:.2f} s)')
axs[0, 1].scatter(range(len(cput_log))[::ite], np.abs(mu_log)[::ite], c='k', s=4,
                  label=f'ITE (avgT={avgT_ite:.2f} s)')
axs[0, 1].set_xlabel(r'Step', fontsize=20)

axs[0, 1].set_xlim(0, 8*1e2)
...
axs[0, 1].text(0.05, 0.85, "(b)", fontsize=28, transform=axs[0, 1].transAxes)

# (c) E error vs CPU time
axs[0, 2].scatter(sweep_cput[::ite], np.abs(sweep_E)[::ite], c='r', s=4)
axs[0, 2].scatter(sweep_cput[799], np.abs(sweep_E)[799], c='r', s=96)

axs[0, 2].scatter(CPU_time_mat[::ite], np.abs(E_mat)[::ite], c='b', s=4)
axs[0, 2].scatter(CPU_time_mat[799], np.abs(E_mat)[799], c='b', s=96)

axs[0, 2].scatter(CPU_time_mat_FD[::ite], np.abs(E_mat_FD)[::ite], c='g', s=4,
                  label=f'BEFD')
axs[0, 2].scatter(CPU_time_mat_FD[799], np.abs(E_mat_FD)[799], c='g', s=96)

axs[0, 2].scatter(cput_log[::ite], np.abs(Etot_log)[::ite], c='k', s=4,
                  label=f'ITE')
axs[0, 2].scatter(cput_log[799], np.abs(Etot_log)[799], c='k', s=96)

axs[0, 2].set_xlabel(r'CPU time (s)', fontsize=20, ha='left', x=-0.05)
axs[0, 2].set_xlim(0, 1e4)
axs[0, 2].set_ylim(2.24, 2.6)
axs[0, 2].set_ylabel(r'$\mathrm{E}$', fontsize=20)
...
axs[0, 2].text(0.05, 0.85, "(c)", fontsize=28, transform=axs[0, 2].transAxes)
axs[0, 2].legend(loc='upper right', fontsize=14, markerscale=2)
...
# -------- unify color scale --------
all_Z = []
for d in [density_gd, density_besp, density_befd, density_bench]:
...
def plot_density(ax, density_data, title, label, show_labels=False):
    ...
    X, Y, Z = density_data
    surf = ax.contourf(X, Y, Z, cmap=cm.coolwarm, vmin=vmin, vmax=vmax)
    ...
    if show_labels:
        ...
        ax.set_ylabel(r'$y$')
        try:
            ps.set_tick_inteval(ax.yaxis, major_itv=10, minor_itv=2)
            ps.set_tick_inteval(ax.xaxis, major_itv=10, minor_itv=2)
        ...
        try:
            ...
            ax.tick_params(axis='both', which='major',
                           labelleft=False, labelbottom=False)
    ax.text(0.05, 0.85, label, transform=ax.transAxes, fontsize=28)
...
# ---- plot four panels ----
plot_density(axs[1, 0], density_gd,   'QTT GD', '(e)', show_labels=True)
plot_density(axs[1, 1], density_besp, 'BESP', '(f)')
plot_density(axs[1, 2], density_befd, 'BEFD', '(g)')
surf_h = plot_density(axs[1, 3], density_bench, 'ITE','(h)')


# -------- Force identical ticks across (e)-(h) --------
# 以 (e) 為基準

xticks = axs[1, 0].get_xticks()
yticks = axs[1, 0].get_yticks()
print(xticks)
'''
for j in range(4): 
    axs[1,j].set_xticks(xticks) 
    axs[1,j].set_yticks(yticks) 
    axs[1,j].set_xlim(-21, 21) 
    axs[1,j].set_ylim(-21, 21)
'''

# -------- Single colorbar only for (h) --------

ticks = [0, 1e-2, 2e-2, 3e-2]
cbar = fig.colorbar(
    surf_h,
    ax=axs[1, 3],
    fraction=0.046,
